...
func New(config *config.Config, model *models.Model, logger *logger.Logger) (*Database, error) {
	...
	case "sqlite":
		...
		if err != nil {
			logger.Error().Err(err).Msg("Failed to enable WAL journal mode")
		} else {
			logger.Info().Msg("Enabled WAL journal mode")
		}

		err = db.Exec("PRAGMA foreign_keys = ON;").Error
		if err != nil {
			logger.Error().Err(err).Msg("Failed to enable foreign keys")
		} else {
			logger.Info().Msg("Enabled foreign keys")
		}
	case "mysql":
		db, err = gorm.Open(
			mysql.Open(
...
func (d *Database) Start() error {
	if d.config.ENV != "prod" && d.config.ENV != "production" {
		d.logger.Info().Msg("Migrating model")
		err := d.model.Migrate(d.DB)
		if err != nil {
			return err
		}

		d.logger.Info().Msg("Seeding model")
